...
namespace EmployeeCount
{
    ...
    public class EmployeeCount : System.Web.Services.WebService
    {
        ...
        public int CountEmployee(string office, int officeId)
        {
            try
            {
                ...
                sqlQuery = "SELECT count(*) as count FROM employee WHERE works_for='Retail' AND office_id IN(" + store(storeList) + ")";
                ...
                while(sdr.Read())
                {
                    count = Int32.Parse(sdr["count"].ToString());
                }
                return count;
            }
            catch (Exception ex)
